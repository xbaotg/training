...
struct ST {
  ...
  void update(int no, int l, int r, int id, int v) {
    if (l == r) {
      st[no] = v;
      return;
    }
    ...
    int m = (l + r) >> 1;
    if (id <= m) {
      update(no << 1, l, m, id, v);
    } else {
      update(no << 1 | 1, m + 1, r, id, v);
    }
  ...
  int query(int no, int l, int r, int u, int v) {
    ...
    int m = (l + r) >> 1;
    return min(query(no << 1, l, m, u, v), query(no << 1 | 1, m + 1, r, u, v));
  }
};
