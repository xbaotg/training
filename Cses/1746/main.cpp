...
int trace(int l, int idx) {
  ...
  if (x[idx] == 0) {
    FOR(i, -1, 1) {
      if (l + i >= 1 && l + i <= m) {
        ans = (ans + trace(l + i, idx + 1)) % MOD;
      }
    }

    return memo[l][idx] = ans;
  } else {
    if (abs(x[idx] - l) <= 1) {
      return memo[l][idx] = trace(x[idx], idx + 1);
    } else {
      return memo[l][idx] = 0;
    }
  }
