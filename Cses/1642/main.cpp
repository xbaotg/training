...
void solve() {
  ...
  for (int i = 0; i < sz(poss) - 1; ++i) {
    int v = poss[i].v, a = poss[i].a, b = poss[i].b;
    int rem = x - v;

    if (rem < 2) continue;
    
    auto k = lower_bound(poss.begin() + i + 1, poss.end(), Pair {rem, -INF, -INF}) - poss.begin();
    for (; k < sz(poss); ++k) {
      if (poss[k].v != rem) {
        break;
      } else {
        int ta = poss[k].a, tb = poss[k].b;
        if (ta != a && ta != b && tb != a && tb != b) {
          ps(a + 1, " ", b + 1, " ", ta + 1, " ", tb + 1);
          return;
        }
      }
    }
