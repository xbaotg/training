...
void compute(int row) {
  ...
  FOR (col, 1, 8) {
    if (!bCol[col] && !mainDiag[row - col + 10] && !coDiag[row + col] && mat[row - 1][col - 1] == '.') {
      bCol[col] = true;
      mainDiag[row - col + 10] = true;
      coDiag[row + col] = true;

      compute(row + 1);

      bCol[col] = false;
      mainDiag[row - col + 10] = false;
      coDiag[row + col] = false;
    }
  }
}
...
void solve() {
  ...
  memset(mainDiag, false, sizeof bCol);
  memset(coDiag, false, sizeof bCol);
