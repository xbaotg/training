...
ll pow_mod(ll a, ll b) { ll ans = 1; while (b) { if (b & 1) ans = mul_mod(ans, a); a = mul_mod(a, a); b >>= 1; }; return ans; };
...
void solve() {
  ...
  int count_odd = count_if(cnt, cnt + 26, [](int x) { return x & 1; });
