...
struct ST {
  ...
  void update(int no, int l, int r, int u, int v, int k) {
    if (u > r || v < l) 
      return;
    ...
    int m = (l + r) >> 1;
    update(no << 1, l, m, u, v, k);
    update(no << 1 | 1, m + 1, r, u, v, k);
  }

  llong get(int no, int l, int r, int i) {
    if (i < l || i > r) {
      return 0LL;
    }
    ...
    int m = (l + r) >> 1;
    return get(no << 1, l, m, i) + get(no << 1 | 1, m + 1, r, i);
  }
};
...
void solve() {
  ...
  while (q--) {
    ...
    if (t == 1) {
      int a, b, u; read(a, b, u);

      st.update(1, 1, n, a, b, u);
    } else {
      int u; read(u);
      ps(st.get(1, 1, n, u), nl);
    }
  }
