...
int main() {
  fast_io(); init();
  int T = 1; if (TESTS) read(T); for (int i = 1; i <= T; ++i) solve();
  endps();
}
//}}
void init() { }

void dfs(vector<vector<int>> &G, vector<bool>& vs, int u) {
  vs[u] = true;
...
void solve() {
  ...
  vector<llong> dis (n + 1, LLONG_MIN);

  dis[1] = 0LL;

  for (int i = 0; i < n - 1; ++i) {
    for (auto &p : G) {
      int u = p.F, v = p.S.F, w = p.S.S;

      if (dis[u] != LLONG_MIN && dis[u] + w > dis[v]) {
        dis[v] = dis[u] + w;
      }
    }
  }

  vector<bool> vs1 (n + 1), vs2 (n + 1);
  ...
  for (auto &p : G) {
    ...
    if (dis[u] != LLONG_MIN && vs1[u] && vs2[u] && dis[u] + w > dis[v]) {
      ps(-1);
      return;
    }
  }
