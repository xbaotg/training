...
long long trace(int i, int j) {
  ...
  return dp[i][j] = trace(i - 1, j) + trace(i, j - 1);
}

string find(int i, int j, long long k) {
  if (i == 0) {
    return string(j, 'b');
  }
  ...
  //pd(i, j, k);
  if (k <= dp[i - 1][j]) {
    return "a" + find(i - 1, j, k);
  } else {
    return "b" + find(i, j - 1, k - dp[i - 1][j]);
  }
}
