...
void update(int id, char c) {
    for (; id <= n; ++bit[id][c - 'a'], id += id & -id);
}

int queryPoint(int id, int c) {
    int ans = 0;
    for (; id; ans += bit[id][c - 'a'], id -= id & -id);
    return ans;
}

int count(int l, int r, int c) {
    return queryPoint(r + 1, c) - queryPoint(l, c);
}
...
int f(int l, int r, char c) {
    ...
    return min(
            n - count(l, m, c) + f(m + 1, r, c + 1),
            n - count(m + 1, r, c) + f(l, m, c + 1)
    );
}
