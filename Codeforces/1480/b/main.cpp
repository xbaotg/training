...
void solve() {
  ...
  pair<int, int> m[n];
  REP(i, 0, n) re(m[i].first);
  REP(i, 0, n) re(m[i].second);

  ll dame = 0;

  REP (i, 0, n) dame += ceil(1.0 * m[i].second / A) * m[i].first;

  // if i-th is monter last kill
  REP (i, 0, n) {
    if (B - dame + m[i].first > 0) {
      ps("YES\n");
      return;
    }
  }
