...
int64 trace(char i, char j, char ca, char cb, int nb) {
  if (i > H) {
    return ca == A && cb == B;
  }
  ...
  if (nb & (1 << j)) {
    if (j == W) {
      return trace(i + 1, 1, ca, cb, nb & ~(1 << j));
    }

    return trace(i, j + 1, ca, cb, nb & ~(1 << j));
  }
...

  {
    ...
    if (j + 1 <= W) {
      bool change = j + 1 == W;
      ret += trace(i + change, change ? 1 : j + 2, ca + 1, cb, (nb & ~(1 << (j + 1))));
    }
  }

  {
    ...
    if (i + 1 <= H) {
      bool change = j == W;
      ret += trace(i + change, change ? 1 : j + 1, ca + 1, cb, nb | (1 << j));
    }
  }

  {
    // 1x1
    bool change = j == W;
    ret += trace(i + change, change ? 1 : j + 1, ca, cb + 1, nb);
  }
...
void solve() {
  ...
  // bitset<1> t(0);
  vector<vector<char>> v(H + 1, vector<char>(W + 1, 0));
  ps(trace(1, 1, 0, 0, 0));
}
