...
void solve() {
  ...
  // 2 4
  if (a[0] > 1) {
    pre.push_back({1LL, 1LL});
    cur += a[0] - 1;
  }

  REP(i, 1, n) {
    if (a[i] - a[i - 1] > 1) {
      pre.push_back({cur, a[i - 1] + 1});
      cur += a[i] - a[i - 1] - 1;
    }
  }

  pre.push_back({cur, a[n - 1] + 1});

  long long k;
  while (q--) {
    re(k);

    // upper_bound return which value greater, 
    // pair, if first is equal, compare second

    auto it = std::lower_bound(all(pre), make_pair(k + 1, 0LL));
    auto p = *prev(it);
