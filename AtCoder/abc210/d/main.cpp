...
void solve() {
  ...
  vector<vector<llong>> dp_l (h + 2, vector<llong> (w + 2, LINF));
  vector<vector<llong>> dp_r (h + 2, vector<llong> (w + 2, LINF));

  for (int i = h; i >= 1; --i) {
    for (int k = 1; k <= w; ++k) {
      dp_l[i][k] = min({ dp_l[i+1][k], dp_l[i][k-1], (llong) a[i][k] + c * i - c * k });
    }
  }

  for (int i = h; i >= 1; --i) {
    for (int k = w; k >= 1; --k) {
      dp_r[i][k] = min({ dp_r[i+1][k], dp_r[i][k+1], (llong) a[i][k] + c * i + c * k });
    }
  }
  ...
  for (int i = 1; i <= h; ++i) {
    for (int k = 1; k <= w; ++k) {
      ...
      if (f != LINF) {
        ret = (llong) a[i][k] - c * i - c * k + f;
      }

      if (s != LINF) {
        ret = min(ret, (llong) a[i][k] - c * i + c * k + s);
      }
