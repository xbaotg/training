...
bool possible(int m) {
  ...
  REP (i, 0, n) pre[0][i] = (i ? pre[0][i-1] : 0) + (a[0][i] <= m);
  REP (i, 0, n) pre[i][0] = (i ? pre[i-1][0] : 0) + (a[i][0] <= m);
  REP (i, 1, n) {
    REP (j, 1, n) {
  ...
  REP (i, k - 1, n) {
    REP (j, k - 1, n) {
      int t = pre[i][j] - (i >= k ? pre[i - k][j] : 0) - (j >= k ? pre[i][j-k] : 0) + (i >= k && j >= k ? pre[i-k][j-k] : 0);
...
void solve() {
  re(n, k);
  REP(i, 0, n) REP(j, 0, n) re(a[i][j]);

  //
  me = k*k / 2;
  int _min = 0, _max = 1e9, ans = 0;
